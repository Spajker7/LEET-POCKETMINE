...
final class UpdateAbilitiesPacketLayer{
	...
	public static function decode(NetworkBinaryStream $packet) : self{
		$layerId = $packet->getLShort();
		$setAbilities = $packet->getLInt();
		$setAbilityValues = $packet->getLInt();
		$flySpeed = $packet->getLFloat();
		$walkSpeed = $packet->getLFloat();
	...
	public function encode(NetworkBinaryStream $packet) : void{
		$packet->putLShort($this->layerId);
		...
		$packet->putLInt($setAbilities);
		$packet->putLInt($setAbilityValues);
		$packet->putLFloat($this->flySpeed ?? 0);
		$packet->putLFloat($this->walkSpeed ?? 0);
	}
}